...
namespace pipeann {
  ...
  void SSDIndex<T, TagT>::do_beam_search(const T *query1, uint32_t mem_L, uint32_t l_search, const uint32_t beam_width,
    ...
    // std::cout << "[observability] -------> beam search" << std::endl;
    ...
    while (k < cur_list_size) {
      ...
      if (!frontier.empty()) {
        if (sector_scratch_idx + frontier.size() > MAX_N_SECTOR_READS)
          break;
        if (stats != nullptr)
          stats->n_hops++;
      ...
      for (auto &frontier_nhood : frontier_nhoods) {
        ...
        // Bounds check: node must fit within the sector we read (size_per_io == SECTOR_LEN).
        if (node_disk_buf < sector_buf ||
            node_disk_buf + this->max_node_len > sector_buf + SECTOR_LEN)
          continue;
        unsigned *node_buf = offset_to_node_nhood(node_disk_buf);
        _u64 nnbrs = (_u64) (*node_buf);
        if (nnbrs > this->range)
          nnbrs = this->range;
        if (nnbrs > MAX_N_EDGES)
          nnbrs = MAX_N_EDGES;
        ...
        for (_u64 m = 0; m < nnbrs; ++m) {
          ...
          if (visited.find(id) != visited.end()) {
            ...
            if (cur_list_size > 0 && dist >= retset[cur_list_size - 1].distance && (cur_list_size == l_search))
              continue;
            Neighbor nn(id, dist, true);
            // variable search_L for deleted nodes.
            // InsertIntoPool can write to retset[cur_list_size]; ensure capacity.
            if (unlikely(cur_list_size >= retset.size())) {
              retset.resize(std::max((size_t)(2 * cur_list_size), (size_t)(cur_list_size + 1)));
            }
