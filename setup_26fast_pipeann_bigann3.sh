...
prepare_ground_truth() {
  if [[ -n "${OVERALL_VECS_PER_STEP:-}" ]] && [[ "${OVERALL_VECS_PER_STEP}" -gt 0 ]]; then
    log "Preparing ground truth (${OVERALL_VECS_PER_STEP} cadence for testing)"
  else
  ...
  local step_gt=1000000
  local num_steps=$((SCALE_M - 1))
  if [[ -n "${OVERALL_VECS_PER_STEP:-}" ]] && [[ "${OVERALL_VECS_PER_STEP}" -gt 0 ]]; then
    step_gt="$OVERALL_VECS_PER_STEP"
    num_steps="${OVERALL_MAX_BATCHES:-10}"
    [[ "$num_steps" -lt 1 ]] && num_steps=10
    log "Ground truth at ${step_gt} cadence (OVERALL_VECS_PER_STEP): gt_0, gt_${step_gt}, ... (${num_steps} steps)"
  else
    # step=1M so each 1M batch uses the next gt: gt_0.bin (after 0), gt_1000000.bin (after 1M), ...
    # num_steps = SCALE_M - 1 so last range stays within GT.
    :
  fi
  local tot_npts=$((BASE_NPTS + num_steps * step_gt))
  build/tests/gt_update "$truth_bin" "$tot_npts" "$BASE_NPTS" 10 "$TRUTHSET_DIR" 0 "$step_gt"
  log "Truthset: $(ls "$TRUTHSET_DIR"/gt_*.bin 2>/dev/null | wc -l) files"
  ls -lh "$TRUTHSET_DIR/gt_0.bin" "$TRUTHSET_DIR/gt_${step_gt}.bin" 2>/dev/null || true
}

run_overall_performance() {
  cd "$REPO_DIR"
  log "Running overall_performance (scale ${SCALE_M}M, step=$OVERALL_STEP)"
  [[ -n "${OVERALL_MAX_BATCHES:-}" ]] && log "Cap: OVERALL_MAX_BATCHES=$OVERALL_MAX_BATCHES (unset for default 100)"
  [[ -n "${OVERALL_MAX_QUERIES:-}" ]] && log "Cap: OVERALL_MAX_QUERIES=$OVERALL_MAX_QUERIES (unset = no limit)"
  [[ -n "${OVERALL_VECS_PER_STEP:-}" ]] && log "Batch size: OVERALL_VECS_PER_STEP=$OVERALL_VECS_PER_STEP vectors (unset = index_npts/step, e.g. 1M)"
...
#   SCALE_M=10 OVERALL_MAX_BATCHES=5 OVERALL_MAX_QUERIES=20000 OVERALL_VECS_PER_STEP=20000 S3_BASE=s3://fuzzydedupe/disk_ann/bigann_base.bvecs S3_QUERY=s3://fuzzydedupe/disk_ann/bigann_query.bbin ./setup_26fast_pipeann_bigann2.sh

# unset OVERALL_MAX_BATCHES OVERALL_MAX_QUERIES OVERALL_VECS_PER_STEP S3_BASE S3_QUERY
