...
DATA_DIR="${DATA_DIR:-$MNT/data/siftsmall}"
INDEX_DIR="${INDEX_DIR:-$MNT/indices/siftsmall}"
INDEX_PREFIX="${INDEX_PREFIX:-$INDEX_DIR/siftsmall}"
...
# Small test params (siftsmall = 10K base, 100 queries)
R="${R:-64}"
L_BUILD="${L_BUILD:-96}"
B_PQ="${B_PQ:-3.3}"     # 26FAST README uses 3.3 for 100M-scale; OK for siftsmall too :contentReference[oaicite:4]{index=4}
M_GB="${M_GB:-4}"
...
# Texmex siftsmall
SIFTSMALL_URL="${SIFTSMALL_URL:-ftp://ftp.irisa.fr/local/texmex/corpus/siftsmall.tar.gz}"
SIFTSMALL_TGZ="${SIFTSMALL_TGZ:-$DATA_DIR/siftsmall.tar.gz}"
...
download_siftsmall() {
  ...
  if [[ ! -f "$SIFTSMALL_TGZ" ]]; then
    log "Downloading siftsmall: $SIFTSMALL_URL"
    wget -c "$SIFTSMALL_URL" -O "$SIFTSMALL_TGZ"
  else
    log "Found existing $SIFTSMALL_TGZ"
  fi

  log "Extracting siftsmall.tar.gz"
  tar -xzf "$SIFTSMALL_TGZ" -C "$DATA_DIR"
...
convert_to_bin() {
  ...
  local base_fvecs="$DATA_DIR/siftsmall_base.fvecs"
  local query_fvecs="$DATA_DIR/siftsmall_query.fvecs"
  local gt_ivecs="$DATA_DIR/siftsmall_groundtruth.ivecs"
  ...
  build/tests/utils/fvecs_to_bin "$base_fvecs"  "$DATA_DIR/siftsmall_base.bin"
  build/tests/utils/fvecs_to_bin "$query_fvecs" "$DATA_DIR/siftsmall_query.bin"
  build/tests/utils/ivecs_to_bin "$gt_ivecs"     "$DATA_DIR/siftsmall_gt.bin"
...
build_index_and_search() {
  ...
  build/tests/search_disk_index float "$INDEX_PREFIX" "$SEARCH_THREADS" "$BEAM_WIDTH" \
    "$DATA_DIR/siftsmall_query.bin" \
    "$DATA_DIR/siftsmall_gt.bin" \
