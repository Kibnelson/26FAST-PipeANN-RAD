...
**Why you see iou-sqp-&lt;pid&gt; instead of pa:search:** Your search uses **io_uring** for async I/O. The thread that actually calls into the block layer (and triggers `block_rq_issue`) is the **io_uring kernel thread** (e.g. `iou-sqp-253931`), not your application thread (`pa:search`). The PID in the comm (253931) is the process that owns the io_uring context — your search process. So **iou-sqp-253931** *is* your search’s disk I/O; you just see it under the kernel thread name.
...
2. **Start your search** (same machine). While it is doing disk I/O, **iou-sqp-&lt;pid&gt;** will appear in the snapshots. The PID will be your search process. You’ll see a histogram: most requests often in a band (e.g. 64–128 µs on NVMe), with a tail to higher latencies.
...
**Summary:** The script answers “for each thread name that issued block I/O, how many requests fell into each latency bucket (µs) in the last 5 seconds?” When your search is running, its I/O shows up under **iou-sqp-&lt;pid&gt;**; that histogram is what to look at for “how fast are my search’s disk reads.”
...
| comm | Meaning |
|------|--------|
| **iou-sqp-&lt;pid&gt;** | io_uring submission-queue poll thread for process &lt;pid&gt;. When your app uses io_uring (e.g. PipeANN search), the kernel thread that **issues** block I/O has this name. The PID is your process (e.g. search_disk_index). **This is your search workload’s disk I/O latency.** |
...
**Important:** The script **clears** the histogram every 5 seconds, so each block is a **rolling 5-second window**: only I/O that completed in that window is counted. When your search is running, look for the snapshots that contain **iou-sqp-&lt;pid&gt;**; that histogram is the block-layer latency for your search. When the search has finished, those snapshots will no longer include iou-sqp (or the count will drop to zero).
...
3. **Confirm the binary has USDT.** You can check in two ways:
   ...
   **Without running (no root):** Inspect the binary for USDT (SystemTap) notes:
   ```bash
   readelf -n build/tests/search_disk_index | grep -A2 stapsdt
   ```
   You will typically see **only** `pipeann` / `io_context`. To list probe names with bpftrace (requires root):
   ```bash
   sudo bpftrace -l 'usdt:'$(pwd)'/build/tests/search_disk_index:pipeann:*'
   ```
...
4. **Attach to probes.** Use the **full path** to the executable (`BINARY`). With the current build only **`io_context`** is in the binary (see **Troubleshooting**). Two ways to get events:
   ...
   **Option A — Two terminals (attach by PID):** USDT by path alone does not attach to processes that start *after* bpftrace. Start the search first, then attach bpftrace with `-p <pid>`. **Timing matters:** the search runs load → warmup → benchmark then exits. If you attach bpftrace too late (e.g. after typing in another terminal), most or all queries may already have run and you will see no events before the process exits. Attach **immediately** after starting the search:
   ...
   ```bash
   export BINARY=$(pwd)/build/tests/search_disk_index
   # Start search in background, attach bpftrace immediately (no delay)
   ./build/tests/search_disk_index float /mnt/nvme/indices/sift/sift 1 32 \
     /mnt/nvme/data/sift/sift_query.bin /mnt/nvme/data/sift/sift_gt.bin \
     10 l2 2 0 10 20 30 40 > /tmp/search_out.log 2>&1 &
   sudo bpftrace -p $! -e "usdt:$BINARY:pipeann:io_context { printf(\"io_context=%lu\n\", arg0); }"
   ```
   ...
   **Option B — Single command:** Run the search binary **directly** as the child of bpftrace with `-c` (no bash wrapper). The **child process must be the binary that contains the USDT probes** (search_disk_index); if you use `-c "bash ..."`, the child is bash and bpftrace reports "No probes to attach". So pass the search binary and its args as the full `-c` argument:
   ...
   ```bash
   # From repo root. -c must be the search_disk_index binary + args (no redirect, no bash).
   # Search output will mix with bpftrace "io_context" lines on the terminal.
   sudo bpftrace -e "usdt:$BINARY:pipeann:io_context { printf(\"io_context=%lu\n\", arg0); }" -c "$BINARY float /mnt/nvme/indices/sift/sift 1 32 /mnt/nvme/data/sift/sift_query.bin /mnt/nvme/data/sift/sift_gt.bin 10 l2 2 0 10 20 30 40"
   ```
   ...
   ```bash
   # Context transitions (USDT — only io_context is in the binary)
   sudo bpftrace -e "usdt:$BINARY:pipeann:io_context { printf(\"context=%lu\n\", arg0); }"

   # Per-query count via uprobe (query_start USDT not in binary). Use the *full* -c command below (no literal "..."). Some bpftrace versions fail with END when using -c, so we print the count every 5s; when the search exits you see the final count.
   export BINARY=$(pwd)/build/tests/search_disk_index
   sudo bpftrace -e 'uprobe:'$BINARY':*pipe_search* { @queries++; } interval:s:5 { printf("queries so far: %lu\n", @queries); }' -c "$BINARY float /mnt/nvme/indices/sift/sift 1 32 /mnt/nvme/data/sift/sift_query.bin /mnt/nvme/data/sift/sift_gt.bin 10 l2 2 0 10 20 30 40"
   ```
   ...
   **Alternative (attach to running process):** Start the search in the background, then attach; when you Ctrl+C bpftrace, it will print the map with the total count:
   ...
   ```bash
   ./build/tests/search_disk_index float /mnt/nvme/indices/sift/sift 1 32 /mnt/nvme/data/sift/sift_query.bin /mnt/nvme/data/sift/sift_gt.bin 10 l2 2 0 10 20 30 40 &
   sudo bpftrace -p $! -e 'uprobe:'$(pwd)'/build/tests/search_disk_index:*pipe_search* { @queries++; } END { printf("total queries: %lu\n", @queries); }'
   ```
